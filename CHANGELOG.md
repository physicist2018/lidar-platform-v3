...
- **Пакет `pkg/lidar/molecular`** — расчёт чисто молекулярного (релеевского)
  ...
  - Высота из дальности `z = r·cos(α)`; линейная интерполяция `T`/`P` из
    модели; выход за пределы модели — кламп к краям с предупреждением в лог.
...
- **Пакет `pkg/smooth/tikhlidar`** — тихоновское сглаживание лидарных сигналов
  ...
  - **23 теста**: сигмоида, калибровка, оператор второй производной
    ...
    пачка (`ω=0` ≡ попрофильно, `ω>0` снижает временную дисперсию), валидация.
