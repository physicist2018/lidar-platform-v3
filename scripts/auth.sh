...
#   ./scripts/auth.sh login    <email> <password>          — вход, сохранение токена
#   ./scripts/auth.sh full     <email> <password>          — регистрация + вход
#   ./scripts/auth.sh token                                — показать сохранённый токен
#
...
main() {
    ...
    case "$cmd" in
        ...
        *)
            ...
            echo "Usage: $0 [register|login|full|token|help]" >&2
            exit 1
            ;;
    esac
}
